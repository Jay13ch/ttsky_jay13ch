...
#include "Vcontroller_wrapper.h" // Generated by Verilator

#define DESIGN      controller_wrapper
#define VDESIGN     Vcontroller_wrapper
#define MAIN_TB     Vcontroller_wrapper_TB
  ...
  while (!gQuit) {
    ...
    for (int i = 0; i < gRefreshLimit; ++i) {
      ...
      if (x >= 0 && x < WINDOW_WIDTH && y >= 0 && y < WINDOW_HEIGHT) {
        ...
        int red   = gRenderDAC ? TB->r() : (((TB->m_core->uo_out & 0b0000'0001) << 7) | ((TB->m_core->uo_out & 0b0001'0000) << 2));
        int green = gRenderDAC ? TB->g() : (((TB->m_core->uo_out & 0b0000'0010) << 6) | ((TB->m_core->uo_out & 0b0010'0000) << 1));
        int blue  = gRenderDAC ? TB->b() : (((TB->m_core->uo_out & 0b0000'0100) << 5) | ((TB->m_core->uo_out & 0b0100'0000) << 0));
        if (gDisableR) red = 0;
